...
output "cidr" {
  value = alltrue([
    var.vpc_cidr == aws_vpc.terraform.cidr_block && aws_vpc.terraform.cidr_block == "192.168.0.0/16",
    var.network_http.cidr == aws_subnet.http.cidr_block && aws_subnet.http.cidr_block == "192.168.1.0/24",
    var.network_db.cidr == aws_subnet.db.cidr_block && aws_subnet.db.cidr_block == "192.168.2.0/24"
  ])
}